...
3. 표본공분산행렬의 문제
4. Shrinkage의 기본 아이디어
5. 실전 검증: 포트폴리오 최적화에서의 Linear Shrinkage
6. 왜 Nonlinear Shrinkage가 필요한가
7. 회전등변 추정량(Rotation-Equivariant Estimator)과 오라클
8. 큰 차원 점근이론: Marchenko-Pastur 방정식
9. Oracle Nonlinear Shrinkage 공식
10. Bona Fide 추정량: 실제로 계산 가능하게 만들기
11. 몬테카를로 시뮬레이션으로 본 성능
12. Linear vs Nonlinear 요약 비교
13. 마무리
...
입니다. $S$는 불편추정량(unbiased estimator)이라는 좋은 성질을 갖고 있습니다. 문제는 **차원 $p$가 표본 크기 $n$에 비해 크거나 같을 때** 발생합니다. 금융시장에서는 이런 상황이 자주 발생합니다. — 수백 개 종목의 리스크를 추정하려는데, 안정적인 시계열은 몇 년 치(수십~수백 개월)밖에 없는 경우가 흔하기 때문입니다. —

## 2. 포트폴리오 최적화 문제의 기본 설정

Shrinkage가 왜 필요한지 제대로 이해하려면, 공분산 행렬 $\Sigma$가 포트폴리오 최적화 문제에서 정확히 어떤 역할을 하는지부터 짚고 넘어가야 합니다. Ledoit & Wolf(2003)가 가정하는 상황은 액티브 매니저가 벤치마크 지수를 이기려고 하는, 가장 표준적인 세팅입니다.

**기본 표기.** 종목 수를 $p$개라 하고, 다음과 같은 벡터/행렬을 정의합니다.

- $w_B$: 벤치마크(예: KOSPI200, S&P500)의 종목별 비중 벡터
- $x$: 매니저가 벤치마크 대비 추가/축소한 **액티브 비중** 벡터 (양수면 초과매수, 음수면 초과매도)
- $w_P = w_B + x$: 실제 포트폴리오 비중
- $\mu = \mathbb{E}[y]$: 종목 기대수익률 벡터, $\Sigma$: 종목 수익률의 공분산 행렬
- $\alpha = \mu - (w_B'\mu)\mathbf{1}$: 벤치마크 대비 **기대 초과수익률** 벡터

_**액티브 비중**이란? 펀드 매니저가 본인의 투자 철학대로 시장 평균과 비교했을 때 얼마나 다르게 투자했는가를 나타내는 값으로 수식으로 표현하면 $$ x = w_P - w_B $$ ($x$가 액티브 비중) 이렇게 표현할 수 있습니다._

매니저의 목표는 벤치마크보다 높은 기대수익을 내면서도(=$x'\alpha$가 큼), 그 과정에서 생기는 위험은 최소화하는 것입니다. 여기서 위험은 벤치마크와의 수익률 차이, 즉 **추적오차(tracking error)**로 측정합니다.

$$
\sigma_E^2 = x'\Sigma x \qquad (\text{추적오차 분산})
$$

이 식이 shrinkage 논의의 출발점입니다 — $\sigma_E^2$는 $\Sigma$를 **직접, 이차형식(quadratic form)으로** 사용합니다. 즉 $\Sigma$ 추정치에 있는 오차는 그대로 $x'\Sigma x$의 오차로 전달되고, 최적화 알고리즘은 이 값을 최소화하려 하기 때문에, $\Sigma$의 특정 조합이 (실제로는 그렇지 않은데) 우연히 작게 추정된 곳이 있다면 알고리즘이 바로 그곳에 가장 크게 베팅하게 됩니다. 이것이 표본공분산행렬 문제의 근본 원인입니다.

**최적화 문제.** 매니저가 목표 초과수익 $g$(전형적으로 연 300bp)를 달성하면서 추적오차를 최소화하는 문제는 다음과 같이 씁니다.

$$
\min_x \; x'\Sigma x \quad \text{s.t.} \quad x'\alpha \ge g,\;\; x'\mathbf{1}=0,\;\; x \ge -w_B,\;\; x \le c\mathbf{1}-w_B
$$

제약조건들은 각각: (전체 비중 합이 1이 되도록 하는) 완전투자 조건 $x'\mathbf{1}=0$, 공매도를 금지하는 롱온리(long-only) 조건 $x \ge -w_B$ (즉 $w_P \ge 0$), 그리고 한 종목에 지나치게 쏠리지 않도록 하는 상한 조건 $x \le c\mathbf{1}-w_B$ (예: 한 종목 비중이 10%를 넘지 못하게)입니다.

**성과 지표: 정보비율(Information Ratio).** 매니저의 실현 성과는 초과수익의 평균을 그 변동성으로 나눈 값으로 요약합니다.

$$
\text{IR} = \frac{\mu_E}{\sigma_E} = \frac{x'\alpha}{\sqrt{x'\Sigma x}}
$$

IR이 높을수록 "위험 대비 초과수익"을 잘 낸다는 뜻입니다. 이 최적화 문제에서 매니저가 실제로 다루는 입력은 $\alpha$(기대 초과수익 예측치)와 $\Sigma$(공분산 행렬 추정치) 두 가지뿐이고, 이 글은 그중 $\Sigma$를 어떻게 더 잘 추정할지에 집중합니다. $\alpha$ 추정(즉 종목을 고르는 능력) 자체는 다루지 않습니다 — 이는 매니저의 리서치 역량에 달린 별개의 문제입니다.

정리하면, 표본공분산행렬 $S$를 그대로 $\hat\Sigma$로 사용하면 위 최적화 문제의 목적함수($x'\Sigma x$)와 제약조건 모두에 추정오차가 반영되고, 그 결과 IR 저하와 회전율(turnover) 증가가 발생합니다.

## 3. 표본공분산행렬의 문제

$p$가 $n$에 비해 크거나 같으면 $S$의 개별 원소들은 심한 추정오차를 갖게 됩니다. 특히 중요한 것은 **고유값(eigenvalue)의 왜곡**입니다: $S$의 가장 큰 고유값들은 실제(모집단) 값보다 체계적으로 더 크게, 가장 작은 고유값들은 더 작게 추정되는 경향이 있습니다. 즉 표본공분산행렬의 고유값 스펙트럼은 실제보다 더 넓게 퍼져 보입니다.

이게 왜 문제냐면, 포트폴리오 최적화 같은 절차는 공분산 행렬의 정보를 그대로 신뢰해서 의사결정을 내립니다. 만약 특정 종목 조합의 분산이 (추정오차 때문에) 우연히 아주 작게 나왔다면, 평균-분산 최적화 알고리즘은 바로 그 조합에 가장 큰 베팅을 겁니다. 즉 **가장 신뢰할 수 없는 추정치에 가장 크게 의존하는** 역설적인 상황이 발생합니다. 이를 Michaud(1989)는 "error-maximization"이라 불렀습니다. 결과적으로 매니저의 IR은 실제 종목 선정 능력보다 체계적으로 낮게 나타납니다.

## 4. Shrinkage의 기본 아이디어

Shrinkage의 핵심 발상은 간단합니다: 극단적인 값으로 치우친 추정치를 좀 더 중심 쪽으로 당겨오자는 것입니다. 통계학적으로 표현하자면 **편향-분산 트레이드오프(bias-variance tradeoff)** 문제입니다.

예를 들어 보겠습니다. 다트를 던지는 두 사람이 있다고 생각해봅시다. 한 명은 평균적으로는 정중앙을 맞히지만(불편) 매번 크게 흩어져 던지는 사람($S$에 해당), 다른 한 명은 항상 비슷한 자리에 몰아 던지지만 정중앙에서 살짝 벗어난 곳을 겨냥하는 사람(구조화된 추정량 $F$에 해당)입니다. 이 둘의 조준을 적절히 평균 내면, 둘 중 어느 한쪽만 쓰는 것보다 더 정확한 결과를 얻을 수 있습니다. Shrinkage는 정확히 이 원리를 공분산 행렬 추정에 적용한 것입니다.

### 4.1 Shrinkage 추정량의 세 가지 구성요소
...
### 4.2 Shrinkage Target: Constant Correlation Model
...
평균을 구하기 위해 전체 합을 개수로 나누어 평균을 내는 식을 써보면 다음과 같습니다.

$$\bar{r} = \frac{\text{전체 합}}{\text{개수}} = \frac{\sum\sum r_{ij}}{p\text{C}_{2}}$$

여기에 $p\text{C}_{2} = \frac{p(p-1)}{2}$를 대입하면 분모의 분수가 뒤집히면서 곱해지게 됩니다.
...
- **Concentration ratio($c=p/n$) 효과**: nonlinear shrinkage는 $c$가 0.1~0.9 전 구간에서 항상 **90% 이상**을 유지합니다. Linear shrinkage는 $c$가 작을 때(데이터가 상대적으로 풍부할 때) 오히려 30%대까지 떨어지고, $c$가 클 때는 93%까지 회복됩니다 — 6절에서 설명한 "역설"이 그대로 확인됩니다.
