...
이 식이 shrinkage 논의의 출발점입니다 — $\sigma_E^2$는 $\Sigma$를 **직접, 이차형식(quadratic form)으로** 사용합니다. 즉 $\Sigma$ 추정치에 있는 오차는 그대로 $x'\Sigma x$의 오차로 전달되고, 최적화 알고리즘은 이 값을 최소화하려 하기 때문에, $\Sigma$의 특정 조합이 (실제로는 그렇지 않은데) 우연히 작게 추정된 곳이 있다면 알고리즘이 바로 그곳에 가장 크게 베팅하게 됩니다. 이것이 3절에서 다룰 표본공분산행렬 문제의 근본 원인입니다.
...
정리하면, 표본공분산행렬 $S$를 그대로 $\hat\Sigma$로 사용하면 위 최적화 문제의 목적함수($x'\Sigma x$)와 제약조건 모두에 추정오차가 스며들고, 그 결과가 바로 5절(실전 검증)에서 보게 될 IR 저하와 회전율(turnover) 증가입니다.
